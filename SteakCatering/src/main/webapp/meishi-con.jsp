...
<%--如果meiShi没有数据，跳转到findDishById.sd--%>
<c:if test="${empty meiShi}">
	<%
		request.getRequestDispatcher("/findDishById.sd").forward(request, response);
	%>
</c:if>
...
<html>
	...
	<body>
		...
		<ul class="breadnav clearfix">
			...
			<li class="bnav-item">&lt;</li>
			<li class="bnav-item"><a href="javascript:;">${meiShi.sdDishName}</a></li>
		</ul>
		<div class="mei-wrap">
			<p class="mei-tit">${meiShi.sdDishName}</p>
			<div class="mei-txt">
				<p>${meiShi.sdDishContent}</p>
			</div>
			<a href="${pageContext.request.contextPath}/dishPageQuery.sd?curPageNo=${curPageNo}" class="meishi-icon">返回列表页</a>
		</div>
